<x-admin-layout mainInnerWrapper="p-0">
    <div class="min-h-full">
        <form method="POST" action="{{ route('admin.research-articles.update', $researchArticle) }}"
                    ...
                    <div class="flex-1 flex flex-col overflow-hidden">
                        ...
                        <div class="mb-6 flex-shrink-0">
                            ...
                            <input type="text" name="title" id="title"
                                value="{{ old('title', $researchArticle->title) }}" required
                        ...
                        <div class="flex-1 flex flex-col min-h-0">
                            ...
                            <textarea name="content" id="content" class="tinymce-editor"
                                required>{{ old('content', $researchArticle->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="excerpt" id="excerpt" rows="4" maxlength="500"
                                ...
                                placeholder="Brief description of the article...">{{ old('excerpt', $researchArticle->excerpt) }}</textarea>
                        ...
                        <div>
                            ...
                            <select name="category_id" id="category_id"
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $researchArticle->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div id="imageDropZone"
                                ...
                                <div id="imagePlaceholder"
                                    class="space-y-2 {{ $researchArticle->featured_image ? 'hidden' : '' }}">
                                ...
                                <div id="imagePreview" class="{{ $researchArticle->featured_image ? '' : 'hidden' }}">
                                    <img id="previewImg"
                                        src="{{ $researchArticle->featured_image ? asset('storage/' . $researchArticle->featured_image) : '' }}"
                                        alt="Preview" class="max-w-full h-auto rounded-lg mb-2">
                        ...
                        <div>
                            ...
                            <select name="tags[]" id="tags" multiple
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $researchArticle->tags->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        {{ $tag->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <option value="draft" {{ old('status', $researchArticle->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="submitted" {{ old('status', $researchArticle->status) == 'submitted' ? 'selected' : '' }}>Submit for Review</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label class="flex items-center">
                                <input type="checkbox" name="is_restricted" value="1" {{ old('is_restricted', $researchArticle->is_restricted) ? 'checked' : '' }}
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <textarea name="meta_description" id="meta_description" rows="3"
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                                        placeholder="SEO meta description...">{{ old('meta_description', $researchArticle->meta->where('meta_key', 'meta_description')->first()->meta_value ?? '') }}</textarea>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="meta_keywords" id="meta_keywords"
                                        value="{{ old('meta_keywords', $researchArticle->meta->where('meta_key', 'meta_keywords')->first()->meta_value ?? '') }}"
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
            ...
            <div class="mt-6 flex items-center justify-between border-t border-gray-200 pt-6">
                <div>
                    <a href="{{ route('admin.research-articles.preview', $researchArticle) }}" target="_blank"
