<x-admin-layout>
    <div class="px-4 py-6 sm:px-0">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">Edit Document</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="regulator" id="regulator" required
                            ...
                            <option value="SEBI" {{ old('regulator', $document->regulator) == 'SEBI' ? 'selected' : '' }}>
                                SEBI</option>
                            <option value="RBI" {{ old('regulator', $document->regulator) == 'RBI' ? 'selected' : '' }}>
                                RBI</option>
                            <option value="OTHER" {{ old('regulator', $document->regulator) == 'OTHER' ? 'selected' : '' }}>OTHER</option>
                        </select>
                    </div>
                            ...
                            @foreach($categories as $category)
                                @foreach($category->children as $subCategory)
                                    <option value="{{ $subCategory->id }}" data-parent="{{ $category->id }}"
                                        data-regulatory-body="{{ $category->regulatory_body }}" {{ old('sub_category_id', $document->sub_category_id) == $subCategory->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="access_type" id="access_type" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="public" {{ old('access_type', $document->access_type) == 'public' ? 'selected' : '' }}>Public</option>
                            <option value="restricted" {{ old('access_type', $document->access_type) == 'restricted' ? 'selected' : '' }}>Restricted</option>
                        </select>
                    </div>

                    <div>
                        <label for="file" class="block text-sm font-medium text-gray-700">Upload New Document
                            (PDF/DOC/DOCX)</label>
                        ...
                        @endphp
                        <p class="mt-1 text-sm text-gray-500">Current file: <a
                                href="{{ route('admin.documents.download', $document) }}"
                                download="{{ $downloadFileName }}"
                                class="text-indigo-600 hover:text-indigo-900">Download</a></p>
                    </div>
                </div>
    ...
    <script>
        ...
        function openCategoryModal() {
            ...
            // Pre-fill regulatory body from main form and make it read-only
            if (regulatoryBody) {
        ...
        document.getElementById('categoryForm').addEventListener('submit', async function (e) {
            ...
            if (data.success) {
                ...
                // Trigger filter to show/hide based on regulatory body
                filterCategories();
