<x-admin-layout>
    <div class="px-4 py-6 sm:px-0">
        ...
        <form method="POST" action="{{ route('admin.policies.store') }}" enctype="multipart/form-data" id="policyForm">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="tagline" id="tagline" rows="3"
                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500"
                            placeholder="Optional one-line summary or quote...">{{ old('tagline') }}</textarea>
                        @error('tagline')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <!-- Icon Selector -->
                    <x-icon-selector name="icon" :value="old('icon', '')" label="Icon" />
                    @error('icon')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
