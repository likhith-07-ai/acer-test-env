<x-admin-layout>
    <div class="px-4 py-6 sm:px-0">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-900">Research Categories</h1>
            @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('research-categories.create'))
            <a href="{{ route('admin.research-categories.create') }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-3 py-2">
                                <span class="text-sm text-gray-900">{{ $category->articles->count() }}</span>
                            </td>
                            <td class="px-3 py-2">
                            ...
                            <td class="px-3 py-2 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    <a href="{{ route('admin.research-categories.edit', $category) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                    ...
                                    <form method="POST" action="{{ route('admin.research-categories.destroy', $category) }}" class="inline" onsubmit="return confirm('Are you sure? This will delete the category and all its sub-categories.');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                No categories found.
                                @if(auth()->user()->isSuperAdmin() || auth()->user()->hasPermission('research-categories.create'))
                                <a href="{{ route('admin.research-categories.create') }}" class="text-indigo-600 hover:text-indigo-900 font-medium">Create one</a>
                                @endif
                            </td>
                        </tr>
                        @endforelse
