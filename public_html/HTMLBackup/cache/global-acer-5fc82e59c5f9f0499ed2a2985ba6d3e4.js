...
class ThemeManager {
    ...
    setupEventListeners() {
        ...
        window.addEventListener('contentLoaded', () => this.initializeElements());
...
class SEOManager {
    ...
    getBaseUrl() {
        ...
        // Method 3: Check for script tag src to determine base path
            ...
            const scripts = document.querySelectorAll('script[src*="/includes/"]');
            if (scripts.length > 0) {
                const scriptSrc = scripts[0].src;
                const includesIndex = scriptSrc.indexOf('/includes/');
                if (includesIndex > 0) {
                    const baseFromScript = scriptSrc.substring(0, includesIndex);
    ...
    async initialize() {
        try {
            console.log('SEO Manager initializing with baseUrl:', this.baseUrl);
            console.log('Template:', this.templateName, 'Page:', this.pageName);
            
            // Skip external file loading if server-rendered metadata is available
            if (window.cmsMetadata && window.cmsMetadata.rendered) {
                console.log('Using server-rendered metadata, skipping external file loading');
                return true;
            }
    ...
    updatePageMetadata() {
        ...
        const processedTemplate = this.processTemplate(this.template, this.metadata);
...
class TemplateManager {
    constructor() {
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => this.updateStorageInfo());
        } else {
    ...
    resetTemplate() {
        try {
            ...
            // Remove cookie
            document.cookie = 'cms_template=;path=/;expires=Thu, 01 Jan 1970 00:00:01 GMT';
